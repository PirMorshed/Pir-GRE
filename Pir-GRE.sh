...
ensure_packages() {
  add_log "Checking required packages: iproute2, iptables"
  render
  local missing=()
...
ExecStart=/sbin/sysctl -w net.ipv4.conf.default.rp_filter=0
ExecStart=/sbin/sysctl -w net.ipv4.conf.gre${id}.rp_filter=0
...
make_fw_service() {
  ...
  # IPTables based port forwarding (Very lightweight, handles TCP & UDP in Kernel-Space)
  cat >"$path" <<EOF
[Unit]
Description=forward gre${id} ${port} (IPTables)
...
RemainAfterExit=yes
ExecStart=/sbin/sysctl -w net.ipv4.ip_forward=1
# Clean potentially duplicate rules first
ExecStart=-/sbin/iptables -t nat -D PREROUTING -p tcp --dport ${port} -j DNAT --to-destination ${target_ip}:${port}
ExecStart=-/sbin/iptables -t nat -D POSTROUTING -p tcp -d ${target_ip} --dport ${port} -j MASQUERADE
ExecStart=-/sbin/iptables -t nat -D PREROUTING -p udp --dport ${port} -j DNAT --to-destination ${target_ip}:${port}
ExecStart=-/sbin/iptables -t nat -D POSTROUTING -p udp -d ${target_ip} --dport ${port} -j MASQUERADE
ExecStart=-/sbin/iptables -D FORWARD -p tcp -d ${target_ip} --dport ${port} -j ACCEPT
ExecStart=-/sbin/iptables -D FORWARD -p udp -d ${target_ip} --dport ${port} -j ACCEPT
# Add Rules
ExecStart=/sbin/iptables -t nat -A PREROUTING -p tcp --dport ${port} -j DNAT --to-destination ${target_ip}:${port}
ExecStart=/sbin/iptables -t nat -A POSTROUTING -p tcp -d ${target_ip} --dport ${port} -j MASQUERADE
ExecStart=/sbin/iptables -t nat -A PREROUTING -p udp --dport ${port} -j DNAT --to-destination ${target_ip}:${port}
ExecStart=/sbin/iptables -t nat -A POSTROUTING -p udp -d ${target_ip} --dport ${port} -j MASQUERADE
ExecStart=/sbin/iptables -I FORWARD -p tcp -d ${target_ip} --dport ${port} -j ACCEPT
ExecStart=/sbin/iptables -I FORWARD -p udp -d ${target_ip} --dport ${port} -j ACCEPT
# Clean on Stop
ExecStop=/sbin/iptables -t nat -D PREROUTING -p tcp --dport ${port} -j DNAT --to-destination ${target_ip}:${port}
ExecStop=/sbin/iptables -t nat -D POSTROUTING -p tcp -d ${target_ip} --dport ${port} -j MASQUERADE
ExecStop=/sbin/iptables -t nat -D PREROUTING -p udp --dport ${port} -j DNAT --to-destination ${target_ip}:${port}
ExecStop=/sbin/iptables -t nat -D POSTROUTING -p udp -d ${target_ip} --dport ${port} -j MASQUERADE
ExecStop=/sbin/iptables -D FORWARD -p tcp -d ${target_ip} --dport ${port} -j ACCEPT
ExecStop=/sbin/iptables -D FORWARD -p udp -d ${target_ip} --dport ${port} -j ACCEPT
...
uninstall_clean() {
  ...
  while true; do
    ...
    echo "  - cron + /usr/local/bin/pir-gre-recreate-gre${id}.sh (if exists)"
    echo "  - /var/log/pir-gre-gre${id}.log (if exists)"
...
recreate_automation() {
  ...
  script="/usr/local/bin/pir-gre-recreate-gre${id}.sh"
...
LOG_FILE="/var/log/pir-gre-gre${ID}.log"
TZ="Europe/Berlin"
...
# Updated for IPTables rules replacement
if [[ "$SIDE" == "IRAN" ]]; then
  while IFS= read -r fw_path; do
    [[ -n "$fw_path" ]] || continue
    sed -i.bak -E "s/(--to-destination )[0-9.]+(:[0-9]+)/\1${peer_ip}\2/g" "$fw_path"
    sed -i.bak -E "s/(-d )[0-9.]+/\1${peer_ip}/g" "$fw_path"
  done < <(list_fw_units)
fi
...
EOF
  ...
  add_log "Log   : /var/log/pir-gre-gre${id}.log"
...
automation_script_path() {
  local id="$1"
  echo "/usr/local/bin/pir-gre-recreate-gre${id}.sh"
}

automation_log_path() {
  local id="$1"
  echo "/var/log/pir-gre-gre${id}.log"
}
...
# Updated for IPTables rules replacement
if [[ "$SIDE" == "IRAN" ]]; then
  for fw_path in /etc/systemd/system/fw-gre${ID}-*.service; do
    [[ -f "$fw_path" ]] || continue
    sed -i.bak -E "s/(--to-destination )[0-9.]+(:[0-9]+)/\1${peer_ip}\2/g" "$fw_path"
    sed -i.bak -E "s/(-d )[0-9.]+/\1${peer_ip}/g" "$fw_path"
  done
...
ensure_root "$@"
add_log "PIR-GRE GRE+FORWARDER installer."
main_menu
